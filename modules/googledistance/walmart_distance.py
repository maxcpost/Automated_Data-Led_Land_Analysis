...
def find_nearest_walmart(latitude, longitude, api_key):
    ...
    try:
        ...
        console.print(f"[red]Error finding nearest Walmart: {e}[/red]")
        return None
...
def get_travel_details(origin_lat, origin_lng, dest_lat, dest_lng, api_key):
    ...
    try:
        ...
        if data['status'] == 'OK' and data['rows'][0]['elements'][0]['status'] == 'OK':
            ...
            console.print(f"[yellow]Failed to get travel details. Status: {data['status']}[/yellow]")
            return None
    except Exception as e:
        console.print(f"[red]Error getting travel details: {e}[/red]")
        return None
            ...
            if walmart_data:
                ...
                if travel_data:
                    ...
                    if update_master_csv(df, index, walmart_data, travel_data):
                        progress.update(task, advance=1, description=f"[cyan]Processing listing {index+1}/{len(rows_to_process)}...")
                    else:
                        progress.update(task, advance=1, description=f"[red]Failed to update listing {index+1}")
                else:
                    progress.update(task, advance=1, description=f"[yellow]No travel data for listing {index+1}")
            else:
                progress.update(task, advance=1, description=f"[yellow]No Walmart found for listing {index+1}")
